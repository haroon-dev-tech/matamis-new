...
if ($selectedCompanyId && user_owns_company($db, $selectedCompanyId, $userId)) {
    ...
    $availableYears = get_available_somci_years($db, $selectedCompanyId, $userId);
...
<div class="mb-6">
    <p class="text-sm text-slate-500 dark:text-slate-400">
        Statement of Comprehensive Income (Overall) — view all monthly SOMCI entries
    </p>
</div>
...
<div class="card mb-6 p-6">
    <form method="GET" class="space-y-4">
        ...
        <div class="flex flex-wrap gap-3">
            ...
            <a href="<?= BASE_URL ?>/soci/index.php" class="btn-secondary">Reset</a>
        </div>
    </form>
</div>
...
<div class="card p-12 text-center">
    <p class="text-slate-500">Register a company and enter SOMCI data first.</p>
    <a href="<?= BASE_URL ?>/companies/create.php" class="btn-primary mt-4">Register Company</a>
</div>
<?php elseif (empty($periodRows)): ?>
<div class="card p-12 text-center">
    <p class="text-slate-500">No SOMCI entries found for the selected filters.</p>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-800/50">
                    ...
                    <th class="px-6 py-3 text-right font-semibold">Gross Profit/Loss</th>
                    <th class="px-6 py-3 text-right font-semibold">Profit/Loss</th>
            ...
            <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
                ...
                <?php $profitable = $row['totals']['profit_loss'] >= 0; ?>
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/30">
                    ...
                    <td class="px-6 py-4 text-right font-mono tabular-nums"><?= format_money($row['totals']['total_revenue']) ?></td>
                    <td class="px-6 py-4 text-right font-mono tabular-nums <?= $row['totals']['gross_profit_loss'] >= 0 ? 'text-emerald-600' : 'text-red-600' ?>">
                        <?= format_money($row['totals']['gross_profit_loss']) ?>
                    </td>
                    <td class="px-6 py-4 text-right font-mono tabular-nums <?= $profitable ? 'text-emerald-600' : 'text-red-600' ?>">
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="table-actions">
                            <a href="<?= BASE_URL ?>/soci/view.php?company_id=<?= $selectedCompanyId ?>&year=<?= (int)$row['period_year'] ?>&month=<?= (int)$row['period_month'] ?><?= $branchId ? '&branch_id=' . $branchId : '' ?>" class="btn-action btn-action-view">View</a>
                            <span class="table-action-sep">|</span>
                            <a href="<?= BASE_URL ?>/somci/entry.php?company_id=<?= $selectedCompanyId ?>&year=<?= (int)$row['period_year'] ?>&month=<?= (int)$row['period_month'] ?><?= $branchId ? '&branch_id=' . $branchId : '' ?>" class="btn-action btn-action-edit">Edit</a>
                        </div>
                    </td>
                </tr>
                <?php endforeach; ?>
