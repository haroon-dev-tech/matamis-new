...
$lineItemKeys = get_all_line_item_keys();
$error = null;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf()) {
        ...
        try {
            $db->beginTransaction();
            $stmt = $db->prepare(
                'INSERT INTO somfp_entries (branch_id, period_year, period_month, line_item_key, amount)
                 VALUES (?, ?, ?, ?, ?)
            ...
            $db->commit();
            flash('success', 'SOMFP data saved for ' . $currentBranch['name'] . ' — ' . MONTHS[$month] . ' ' . $year);
            redirect('/somfp/entry.php?company_id=' . $companyId . '&branch_id=' . $branchId . '&year=' . $year . '&month=' . $month);
        } catch (Exception $e) {
            $db->rollBack();
...
$groupMap = [];
foreach ($somfpConfig['sections'] as $section) {
    foreach ($section['groups'] as $gk => $g) {
        $groupMap[$gk] = array_keys($g['items']);
    }
}

$pageTitle = 'SOMFP — ' . $company['name'];
...
<div class="mb-6 flex flex-wrap items-center justify-between gap-4">
    ...
    <a href="<?= BASE_URL ?>/somfp/view.php?company_id=<?= $companyId ?>&year=<?= $year ?>&month=<?= $month ?>" class="btn-secondary text-sm">Consolidated View</a>
</div>
...
<div class="mb-4">
    ...
    <p class="text-sm text-slate-500"><?= e(MONTHS[$month]) ?> <?= $year ?> · Statement of Financial Position (Monthly)</p>
</div>
...
<form method="POST" id="somfp-form" class="card overflow-hidden"
      data-group-map='<?= e(json_encode($groupMap)) ?>'
      data-asset-groups="fixed_assets,current_assets"
      data-el-groups="equity,non_current_liabilities,current_liabilities">
    <?= csrf_field() ?>
    <input type="hidden" name="company_id" value="<?= $companyId ?>">
    ...
    <div class="border-t border-slate-200 px-6 py-4 dark:border-slate-800">
        <div class="flex justify-end gap-3">
            <a href="<?= BASE_URL ?>/somfp/index.php?company_id=<?= $companyId ?>&year=<?= $year ?>&month=<?= $month ?>" class="btn-secondary">Cancel</a>
            <button type="submit" class="btn-primary">Save SOMFP Data</button>
        </div>
    </div>
</form>
