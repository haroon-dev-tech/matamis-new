<?php
$pageTitle = 'SOMCI Consolidated View';
$activeNav = 'somci';
$requireAuth = true;
...
if ($branchId) {
    ...
    if (!$branch) {
        ...
        redirect('/somci/index.php?company_id=' . $companyId);
    }
    ...
    $consolidated = $values;
    $pageTitle = 'SOMCI — ' . $company['name'] . ' — ' . $branch['name'];
} else {
    $branchData = [];
    $consolidated = [];
    ...
    $pageTitle = 'SOMCI — ' . $company['name'] . ' (Consolidated)';
}
...
<div class="mb-6 flex flex-wrap items-center justify-between gap-4">
    ...
    <a href="<?= BASE_URL ?>/somci/entry.php?company_id=<?= $companyId ?>&year=<?= $year ?>&month=<?= $month ?><?= $branchId ? '&branch_id=' . $branchId : '' ?>" class="btn-primary text-sm">Enter / Edit Data</a>
</div>

<div class="mb-6">
    <h2 class="text-lg font-semibold"><?= e($company['name']) ?></h2>
    <p class="text-sm text-slate-500">
        <?= e(MONTHS[$month]) ?> <?= $year ?> · Statement of Monthly Comprehensive Income
...
<div class="card p-12 text-center">
    <p class="text-slate-500">No SOMCI data found for this period.</p>
    <a href="<?= BASE_URL ?>/somci/entry.php?company_id=<?= $companyId ?>&year=<?= $year ?>&month=<?= $month ?>" class="btn-primary mt-4">Enter SOMCI Data</a>
</div>
<?php else: ?>
    ...
    <div class="flex items-center justify-between border-b border-slate-200 px-6 py-4 dark:border-slate-800">
        ...
        <div class="flex items-center gap-4 text-sm">
            ...
            <div class="table-actions">
                <a href="<?= BASE_URL ?>/somci/view.php?company_id=<?= $companyId ?>&year=<?= $year ?>&month=<?= $month ?>&branch_id=<?= $bd['branch']['id'] ?>" class="btn-action btn-action-view">View</a>
