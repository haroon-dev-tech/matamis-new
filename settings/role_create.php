        ...
        if ($name === '') {
            ...
            try {
                ...
                foreach ($permissions as $p) {
                    ...
                    if ($canRead || $canWrite) {
                        $ins->execute([$roleId, (int) $p['id'], $canRead, $canWrite]);
                    }
                }
...
<div class="mb-6">
    <p class="text-sm text-slate-500 dark:text-slate-400">Create a role and define read/write/both permissions.</p>
</div>
    ...
    <form method="POST" class="space-y-6">
        ...
        <div class="grid gap-4 sm:grid-cols-2">
            <div>
                ...
                <input type="text" name="name" class="input-field" value="<?= e($_POST['name'] ?? '') ?>" required>
            </div>
            <div>
                <label class="mb-1.5 block text-sm font-medium">Description (optional)</label>
                <input type="text" name="description" class="input-field" value="<?= e($_POST['description'] ?? '') ?>">
            </div>
        </div>
                    ...
                    <table class="w-full text-sm">
                        ...
                        <tbody class="divide-y divide-slate-200 dark:divide-slate-800">
                            ...
                            <tr>
                                ...
                                <td class="px-4 py-3">
                                    <?php $selected = $_POST['access'][$key] ?? 'none'; ?>
        ...
        <div class="flex flex-wrap items-center gap-3">
            ...
            <a href="<?= BASE_URL ?>/settings/roles.php" class="btn-secondary">Cancel</a>
        </div>
    </form>
</div>
